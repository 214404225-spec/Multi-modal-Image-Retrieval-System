...
#### Qwen-VL 与细粒度视觉理解的契合度

Qwen-VL 技术报告中强调的"细粒度视觉理解"（fine-grained visual understanding）与本系统中 VL 精排的属性验证任务高度匹配：

1. **属性验证本质是细粒度视觉理解**：VL 精排阶段验证的颜色（"棕色"）、姿态（"站立"、"奔跑"）、场景（"室内"）、视角（"特写"）等属性，正是报告中定义的细粒度视觉理解任务——不是简单的物体识别，而是对物体细节状态的判断。Qwen3-VL 在训练中采用高分辨率和细粒度语料，使其具备区分细微视觉差异（如"站立"与"蹲坐"的腿部姿态差异）的能力，这对 `complex_attrs` 的验证精度有关键影响。

2. **细粒度训练语料覆盖属性描述**：报告指出的"细粒度语料"意味着模型在训练中见过大量富含属性描述的数据（如"一只棕色奔跑的狗"而非仅"一只狗"），使得 Qwen3-VL 在构建 VL 提示词做二分类验证时，具备更强的语义对齐能力——它天然理解"棕色的狗"和"站立的狗"这些属性组合的含义。

3. **需正视能力边界**：Qwen-VL 的"极具竞争力"是在开源/同规模模型中的相对评价。CLAUDE.md 已指出 VL 精排精度仍不及 SOTA API（Gemini-Flash、GPT-4o），复杂属性组合仍存在漏判。两级差距并存：属性提取能力（Qwen3-VL 在 8B 量级中领先）与绝对精度（与闭源 API 仍有距离）。
