...
- **精排**（`VLRefiner.refine()`）：VL 统一验证。有物体数量则验证计数（"恰好有 N 个 {类别}"），有属性则合并验证（"恰好有 N 个 {属性}的{类别}"），属性仅验证复杂属性（姿态/场景/视角等），简单属性（颜色/大小/明暗）已由 CLIP-only 路径处理
...
| 延迟（精排） | 每张 ~10-20s（本地推理） | 每张 ~0.1-1s（网络 + 推理） |
...
**VL 计数验证耗时较长**：物体数量验证需逐候选调用 VL（每张 ~15s），30 个候选 ≈ 7.5 分钟。**解决方案：换用更快的 VL 模型或云端 API，或减少候选池大小**。
