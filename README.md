...
初版采用人工路由：用户查询经 Qwen2.5-3B 意图识别后，由 LLM 自行选择检索路径。细粒度路径中，VL 模型（Qwen2.5-VL-3B）在离线建库阶段处理所有图像，产出通用增强表示后存入向量库，再与 CLIP 文本编码做相似度匹配。
...
**2. 路由决策程序化**：初版由 LLM 自行选择检索路径（小模型路由不可靠，存在 ~5% 路径选择错误），新版改为 `if attributes:` 确定性代码路由——LLM 只负责提取结构化意图，路由决策完全由代码执行，消除了路由级错误。

**3. VL 精排模型升级**：Qwen2.5-VL-3B → Qwen3-VL-8B，属性二分类误判率降低约 40%（从 ~7% 错误率降至 ~4%）。

**4. VL 使用范式升级：离线盲增强 → 在线靶向验证**。这是与模型升级正交的核心架构创新：
...
即让 VL 输出 0–1 的属性匹配分数，与 CLIP 粗排分加权融合，同时用阈值剔除低分候选。

实际落地时，**本地部署的 qwen3-vl:8b（8B 参数）能力不足以支撑可靠的数值评分**：小模型缺乏校准的置信度输出能力，即使 prompt 明确要求 0–1 分数，也会倾向于输出极端值（几乎总是 1.0 或 0.0），标定不一致且在不同类别、不同属性组合下波动很大。这导致两个后果：

1. **VL 分数退化为二分类** — 模型输出的数值分数无区分度（几乎全是 1.0 / 0.0），继续对它加权融合只会污染 CLIP 粗排分
2. **阈值失去意义** — 在模型输出极端化的前提下，任何居中阈值都没法充分发挥作用，调参也变得不可能

与其让模型输出不可靠的连续分数再手工设阈值，不如明确要求模型做二分类（是/否），把任务匹配到模型真正能可靠执行的水平上。最终简化为当前方案：**VL 只做二分类（是/否），不输出分数**。VL 通过 → 保留原始 CLIP 粗排分；VL 不通过 → 直接剔除。三个参数已从函数签名中移除。
